...
//  public BargeThreadPools() {
//    this(Optional.<NioEventLoopGroup>absent(), Optional.<ListeningExecutorService>absent());
//  }
  
  public BargeThreadPools(String key, Optional<NioEventLoopGroup> eventLoopGroup,
    ...
    if (eventLoopGroup.isPresent()) {
      ...
      this.raftEventLoop = new NioEventLoopGroup(1, new DefaultThreadFactory("pool-raft-" + key));
...
//    this.raftExecutor = MoreExecutors.listeningDecorator(this.raftEventLoop);
    this.raftExecutor = MoreExecutors.listeningDecorator(Executors
        .newSingleThreadScheduledExecutor(new DefaultThreadFactory("pool-raft-executor-" + key)));
